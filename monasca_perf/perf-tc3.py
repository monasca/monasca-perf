...
7/2015 Allan changes
- modified to more closely match HOS metrics on testcluster3
"""
...
urls = [
    'http://mon-ae1test-monasca07.useast.hpcloud.net:8080/v2.0/metrics',
    'http://mon-ae1test-monasca08.useast.hpcloud.net:8080/v2.0/metrics',
    'http://mon-ae1test-monasca09.useast.hpcloud.net:8080/v2.0/metrics',
]

keystone = 'http://10.22.156.20:5002/v3/auth/tokens'
...
def getStatus(ourl,id,x):
    try:
        ...
        for i in xrange(num_metrics_per_request):
            ...
            body.append({"name": "tc.test_perf_" + str(x),
                         "dimensions": {"service": "monitoring", "hostname": "PADAWANBASE-CCP-" + str(i) + "-NETCLM", "component": "persister", "url": "http://PADAWANBASE-CCP:" + str(i)},
