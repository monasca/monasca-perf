...
import logging
import random
from monascaclient import ksclient as monascaclient_ksclient
from monascaclient import client as monascaclient_client
...
def wait_for_all_alarms_to_transition(client, state):
    ...
    while not all_transitioned:
        metrics = [create_metric(str(id), 0.0)
                   for id in xrange(NUM_METRICS_PER_TRIAL)]
...
for trial in xrange(NUM_TRIALS):
    ...
    logger.info(
        'wait for 120 seconds + a random time between 0 and 59 seconds')
    start_time = time.time()
    wait_time = 120 + random.randint(0,
                                     THRESHOLD_ENGINE_EVALUATION_PERIOD_SECS
                                     - 1)
    while True:
        metrics = [create_metric(str(id), 0.0) for id in
                   xrange(NUM_METRICS_PER_TRIAL)]
        send_metrics(monasca_client, metrics)
    ...
    start_time = time.time()
    start_transition_times = [start_time + random.randint(
        0, THRESHOLD_ENGINE_EVALUATION_PERIOD_SECS - 1) for i in xrange(
        NUM_METRICS_PER_TRIAL)]
    ...
    logger.info(
        'randomly transition metrics and wait for all alarms to transition '
        'to ALARM')
    count = 0
    last_collection_time = time.time() - COLLECTION_PERIOD_SECS
    while True:
        current_time = time.time()
        metric_values = map(lambda x: 1.0 if current_time >= x else 0.0,
                            start_transition_times)
        metrics = [create_metric(str(id), metric_values[id]) for id in
                   xrange(NUM_METRICS_PER_TRIAL)]
...
print 'num metrics per trials: ' + str(NUM_METRICS_PER_TRIAL)
print 'mean_of_means: ' + str(mean_of_means)
print 'mean_of_medians: ' + str(mean_of_medians)
print 'mean_of_stds: ' + str(mean_of_stds)
