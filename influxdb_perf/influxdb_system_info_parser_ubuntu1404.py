import re

monitoring = ['influxd', 'pypy']
watched_processes = monitoring

cpu = {'user': [],
       'sys': [],
       'idle': [],
       'wait': []}

memory = {'free': [],
          'buffers': [],
          'cache': []}

samples_per_average = 60
system_memory = 125


def avg(l):
    if len(l) == 0:
        return 0
    return round((sum(l) / len(l)), 2)


def build_avg_set(l):
    return [avg(l[x:x + samples_per_average]) for x in xrange(0, len(l), samples_per_average)]


def min_free(node):
    free = min(node['system_mem']['free'])
    buffers = min(node['system_mem']['buffers'])
    cache = min(node['system_mem']['cache'])
    return free + buffers + cache


def max_used(node):
    return system_memory - min_free(node)


def normalize_memory(mem):
    result = re.match("(.*?)m", mem)
    if result:
        return float(result.group(1)) * 1024

    result = re.match("(.*?)g", mem)
    if result:
        return float(result.group(1)) * 1024 * 1024

    result = re.match("(.*?)t", mem)
    if result:
        return float(result.group(1)) * 1024 * 1024 * 1024

    return mem


def aggregate_process_data(node_data, process_data):
    for k, v in process_data.iteritems():
        data = node_data.get(k, {'cpu': [], 'mem': []})
        data['cpu'].append(sum(v['cpu']))
        data['mem'].append(sum(v['mem']))
        node_data[k] = data


def parse_top_file(node_tuple):
    try:
        node_id, path = node_tuple
        node_data = {'system_cpu': {'user': [],
                                    'sys': [],
                                    'idle': [],
                                    'wait': []},
                     'system_mem': {'free': [],
                                    'buffers': [],
                                    'cache': []}}

        process_data = {}
        with open(path, 'r') as f:
            for line in f:
                line = line.rstrip()

                if not line:
                    continue

                result = re.match("top -", line)
                if result:
                    aggregate_process_data(node_data, process_data)
                    process_data = {}

                result = re.match("Tasks:", line)
                if result:
                    continue

                result = re.match("%Cpu.*?(\d+.\d+) us.*?(\d+.\d+) sy.*?(\d+.\d+) id.*?(\d+.\d+) wa", line)
                if result:
                    node_data['system_cpu']['user'].append(float(result.group(1)))
                    node_data['system_cpu']['sys'].append(float(result.group(2)))
                    node_data['system_cpu']['idle'].append(float(result.group(3)))
                    node_data['system_cpu']['wait'].append(float(result.group(4)))
                    continue
                result = re.match("KiB Mem :.*?(\d+).*?(\d+).*?(\d+).*?(\d+).*", line)

                if result:
                    node_data['system_mem']['free'].append(float(result.group(2)) / 1024 / 1024)
                    node_data['system_mem']['buffers'].append(float(result.group(4)) / 1024 / 1024)
                    continue

                result = re.match("KiB Swap:.*?(\d+).*?(\d+).*?(\d+).*?(\d+).*", line)
                if result:
                    node_data['system_mem']['cache'].append(float(result.group(4)) / 1024 / 1024)
                    continue

                result = re.match("^\s*\d", line)
                if result:
                    if len(line.split()) == 12:
                        pid, user, _, _, virt, res, shr, _, cpup, mem, _, cmd = line.split()

                    if cmd in watched_processes:
                        res = normalize_memory(res)
                        data_points = process_data.get(cmd, {'cpu': [], 'mem': []})
                        data_points['cpu'].append(float(cpup))
                        data_points['mem'].append(float(res))
                        process_data[cmd] = data_points
            else:
                aggregate_process_data(node_data, process_data)

    except Exception:
        import traceback
        print("error on file: {}".format(path))
        traceback.print_exc()

    return (node_id, node_data)


def process_group_report(processes, node1):
    print("{:<10}| {:^8}".format("CPU", "Node"))
    print("--------------------")
    total_cpu = {'node1': [], 'node2': [], 'node3': []}
    for k in processes:
        if k not in node1:
            print("{} not found on all nodes".format(k))
            continue
        n1 = avg(node1[k]['cpu'])
        total_cpu['node1'].append(n1)

        print("{:<10}| {:>8.2f}".format(k, n1))
    print("--------------------")
    print("{:<10}| {:>8.2f}"
          .format('total', sum(total_cpu['node1'])))
    print("\n")
    print("{:<10}| {:^8}".format("MEM", "Node"))
    print("--------------------")
    total_mem = {'node1': [], 'node2': [], 'node3': []}
    for k in processes:
        if k not in node1: # or k not in node2 or k not in node3:
            print("{} not found on all nodes".format(k))
            continue
        total_mem['node1'].append(max(node1[k]['mem']))
        print("{:<10}| {:>8.2f}"
              .format(k, max(node1[k]['mem']) / 1024))
    print("--------------------")
    print("{:<10}| {:>8.2f}".format('total', sum(total_mem['node1']) / 1024))


def generate_report(node1):
    print("{:<10}| {:^8}".format("SYSTEM", "Node"))
    print("--------------------")
    print("{:<10}| {:>8.2f}".format("idle %", avg(node1['system_cpu']['idle'])))
    print("{:<10}| {:>8.2f}".format("min free", min_free(node1)))
    print("{:<10}| {:>8.2f}".format("max used", max_used(node1)))

    for name, group in [('Monitoring', monitoring)]:
        print("\n")
        print("-- {} ------".format(name))
        process_group_report(group, node1)


def parallel_parse(data_path):
    nodes = [('node 1', data_path + "/system_info")]

    result = parse_top_file(nodes[0])
    node_id, data = result
    generate_report(data)


parallel_parse('/home/ubuntu/monasca-perf/influxdb_perf')
